...
class InteractionComplex:
    ...
    def _establish_structure_mappping(self):
        ...
        if 0 in serial_to_ob and 0 not in serial_to_bio:
            offset = 1
            logging.debug(
                """OB atom serial numbers start with 0, but there are no BioPython atoms 
                with serial number 0. Adding 1 to all OB serials for mapping purposes."""
            )
        else:
            offset = 0
        ...
        for serial, ob_atom in serial_to_ob.items():
            ...
            try:
                ...
                raise OBBioMatchError(
                    f"""Failed to match OB atom to a BioPython atom: {serial},
                    {serial + offset} : {ob_atom.GetType()[:1]}({ob_atom.GetType()})"""
                )

            # Sanity check that elements match, for debugging
            if ob_atom.GetType()[:1] != biopython_atom.element[:1]:
                raise OBBioMatchError(
                    f"""Failed to correctly match atoms with different elements: {serial},
                    {serial + offset} : {ob_atom.GetType()[:1]}({ob_atom.GetType()}) != 
                    {biopython_atom.element[:1]}({biopython_atom.element}, {biopython_atom.name})"""
                )
